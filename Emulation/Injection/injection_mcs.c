...
#define PAYLOAD_LEN 3500

uint8_t* conversion(int scrambler);
...
int main(int argc, char *argv[]) {
    ...
    unsigned int ntimes;//number of iterations of scrambler seed from 0-127
    unsigned int npackets = 127*8;
    int i;
	int c;
    ...
    int scrambler = 1;// define start scrambler
    // Session ID
    uint32_t session_id;
    ...
    mcs_iter = 1; //set mcs here 15 for 64QAM 5/6 for emulation
    printf("\n[.]\tMCS %u %s %s\n",
    ...
    for (ntimes = 0; ntimes <= 1; ntimes++ ) {
        ...
        for (count = 1; count <= 127; count++) {
            ...
            memset(encoded_payload, 0, 54);
            addr = conversion(scrambler);
            for (i=0;i<=1700;i++)
            {
            ...
            */
            ...
            lcpf_qos_data(metapack,1, 130,smac, bmac,
                    smac,bmac,1000,100000);//only beacon_interval will impact the payload
...
\x05\x2f\x01\x05\x32\x04\x0c\x12\x18\x60\xdd\x05\x00\x10");
            ...
            //count_255 = count/255 + 1;
            uint8_t Nrounds = ntimes;
            data_inject_main[10] = frame_ind;
            
            data_inject_main[11] = Nrounds;
            
            //printf("Frame_ind = %c\n",frame_ind);
            
            lcpa_append_copy(metapack,"IETAG",1472,data_inject_main);
            ...
            //if (lorcon_send_bytes(context,sizeof(data_inject_main),data_inject_main) < 0 )
            //{
              //  printf("Injection failed1"); 
               // return -1;
            //}
            
            if (lorcon_inject(context,txpack) < 0 )
            {
                printf("Injection failed2"); 
                return -1;
            }
...
uint8_t* conversion(int scrambler)
{
    ...
    //printf("ic = %s\n",ic);
    data_inject = malloc(sizeof(char)*16000);
    //for (file_num = 1;file_num<2;file_num++)
    //{
    char File_name[] = "MAC_PayloadXXX.txt";
    File_name[11] = scrambler/100 + '0';
    File_name[12] = scrambler/10 - (scrambler/100)*10 + '0';
    File_name[13] = scrambler%10 + '0';
    puts(File_name);
    printf("scrambler seed = %d\n",scrambler);
